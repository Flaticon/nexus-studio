...
export default function TalentPage() {
  ...
  const initiatives = [
    ...
    },
    {
      ...
      requiredSkills: [
        ...
        { name: 'Medical Device Integration', priority: 'low', currentCoverage: 0 }
      ],
      currentTeam: ['5', '8'],
  ...
  const talentBank = [
    ...
    },
    {
      ...
      name: 'Lisa Anderson',
      role: 'Data Scientist',
      skills: ['Python', 'R', 'Machine Learning', 'Statistics', 'Visualization'],
      availability: 100,
      experience: 'Mid',
  ...
  const workloadData = teamMembers.map(member => ({
    ...
    workload: member.workload === 'high' ? 90 : member.workload === 'normal' ? 65 : 40,
  ...
  const getSkillMatchForInitiative = (memberId, initiativeId) => {
    ...
    const requiredSkills = initiative.requiredSkills.map(s => s.name.toLowerCase());
  ...
  const filteredMembers = teamMembers.filter(member => {
    const teamMatch = selectedTeam === 'all' || member.currentStartups.includes(selectedTeam);
    const skillMatch = filterSkill === 'all' || member.skills.some(skill => 
      skill.category === filterSkill || skill.name.toLowerCase().includes(filterSkill.toLowerCase())
    );
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
          ...
          <div className="flex gap-3">
            <button 
              ...
              className={`px-4 py-2 border rounded-lg flex items-center gap-2 transition-colors ${
                showSkillMatching ? 'bg-blue-50 border-blue-300 text-blue-700' : 'border-gray-300 hover:bg-gray-50'
              }`}
            >
            ...
            <button 
              ...
              className={`px-4 py-2 border rounded-lg flex items-center gap-2 transition-colors ${
                showTalentBank ? 'bg-purple-50 border-purple-300 text-purple-700' : 'border-gray-300 hover:bg-gray-50'
              }`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Colaboradores Activos</p>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Performance Promedio</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Listos para Rotación</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
      ...
      {showSkillMatching && (
        <div className="bg-white rounded-lg shadow-sm border mb-8">
          <div className="p-6 border-b">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">Match de Skills por Iniciativa</h3>
              <select 
                ...
                return (
                  <div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      {/* Required Skills */}
                      <div>
                        <h4 className="font-medium text-gray-900 mb-4">Skills Requeridos</h4>
                                ...
                                <div className="w-20 bg-gray-200 rounded-full h-2 mt-1">
                                  <div 
                                    className={`h-2 rounded-full ${
                                      skill.currentCoverage >= 80 ? 'bg-green-500' :
                                      skill.currentCoverage >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                                    }`}
                                    style={{ width: `${skill.currentCoverage}%` }}
                                  ></div>
                                </div>
                              </div>
                            </div>
                      ...
                      {/* Best Matches */}
                      <div>
                        <h4 className="font-medium text-gray-900 mb-4">Mejores Candidatos</h4>
                        <div className="space-y-3">
                          {teamMembers
                            .filter(member => !initiative.currentTeam.includes(member.id))
                            .map(member => ({
                              ...member,
                              matchScore: getSkillMatchForInitiative(member.id, initiative.id)
                            }))
      ...
      {showTalentBank && (
        <div className="bg-white rounded-lg shadow-sm border mb-8">
          <div className="p-6 border-b">
            <h3 className="text-lg font-semibold text-gray-900">Banco de Talentos</h3>
            <p className="text-sm text-gray-600 mt-1">Colaboradores disponibles para nuevas iniciativas</p>
          </div>
          <div className="p-6">
              ...
              {talentBank.map(person => (
                <div key={person.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full ${getRotationReadinessColor(person.rotationReadiness)}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border">
          <div className="p-6 border-b">
            <h3 className="text-lg font-semibold text-gray-900">Distribución de Carga de Trabajo</h3>
          </div>
          <div className="p-6">
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={workloadData}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey="workload" fill="#EF4444" name="Carga de Trabajo %" />
        ...
        <div className="bg-white rounded-lg shadow-sm border">
          <div className="p-6 border-b">
            <h3 className="text-lg font-semibold text-gray-900">Distribución de Skills</h3>
          </div>
          <div className="p-6">
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="p-6 border-b">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">Directorio de Colaboradores</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredMembers.map(member => (
                <div key={member.id} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="text-xs text-gray-500 mb-1">Iniciativas Actuales</div>
                      <div className="flex flex-wrap gap-1">
                    ...
                    <div className="flex justify-between text-sm">
                      <div>
                        ...
                        <span className="font-medium text-gray-900 ml-1">{member.availability}%</span>
                      </div>
                      <div>
                        <span className="text-gray-600">Performance:</span>
                        <span className="font-medium text-gray-900 ml-1">{member.performance}%</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between text-sm">
                      <div>
                        ...
                        <span className="font-medium text-gray-900 ml-1">{member.participationScore}%</span>
                      </div>
                      <div>
                        <span className="text-gray-600">Rotación:</span>
                        <span className={`px-1 py-0.5 text-xs rounded ${getRotationReadinessColor(member.rotationReadiness)}`}>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="text-left py-3 px-4 font-medium text-gray-900">Iniciativas</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-900">Skills Principales</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-900">Disponibilidad</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-900">Performance</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-900">Rotación</th>
                ...
                <tbody>
                  {filteredMembers.map(member => (
                    <tr key={member.id} className="border-t hover:bg-gray-50">
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-wrap gap-1">
                          {member.currentStartups.slice(0, 2).map((startup, idx) => (
                            <span key={idx} className="px-2 py-1 text-xs bg-green-50 text-green-700 rounded">
                              {startup}
                            </span>
                          ))}
